    ...
    return (
      <div className="container mx-auto px-4 py-12 min-h-[60vh] flex flex-col items-center justify-center">
        <div className="text-center space-y-4">
          <h2 className="text-2xl font-semibold text-gray-900">⚠️ {result.detail}</h2>
          <p className="text-sm text-gray-500">
            에러 코드: <code className="px-2 py-1 bg-gray-100 rounded">{result.errorCode}</code>
          </p>
          <Link
            href="/products"
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <nav className="text-sm text-gray-500 mb-6" aria-label="breadcrumb">
        <ol className="flex items-center gap-2">
          ...
          <li className="text-gray-900 font-medium truncate max-w-[200px]">{product.productName}</li>
        </ol>
      </nav>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white rounded-2xl shadow-sm border p-6 md:p-8">
        ...
        <div className="relative aspect-square bg-gray-100 rounded-xl overflow-hidden">
          {product.imageUrl ? (
            // ✅ 실제 프로젝트에서는 Next/Image 사용 권장 (lazy loading, optimization)
            <img
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3 mb-8 text-sm text-gray-600 border-t border-gray-100 pt-4">
              <InfoRow label="재고" value={product.stock > 0 ? `${product.stock.toLocaleString()}개` : '품절'} valueColor={product.stock === 0 ? 'text-red-500 font-semibold' : 'text-green-600 font-medium'} />
              <InfoRow label="등록일" value={formatDate(product.createdAt)} />
              <InfoRow label="최근 수정" value={formatDate(product.updatedAt)} />
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-100">
            ...
            >
              {product.stock === 0 || product.status !== 'SELLING' ? '구매 불가' : '구매하기'}
            </button>
            <button
...
function StatusBadge({ status }: { status: ProductStatus }) {
  const config = {
    SELLING: { label: '판매중', className: 'bg-green-100 text-green-800 border-green-200' },
    SOLD_OUT: { label: '품절', className: 'bg-gray-100 text-gray-600 border-gray-200' },
    INACTIVE: { label: '비활성화', className: 'bg-red-100 text-red-800 border-red-200' },
  };
  const { label, className } = config[status] || config.SOLD_OUT;

  return (
    <span className={`px-3 py-1 text-xs font-semibold border rounded-full ${className}`}>
      {label}
    </span>
  );
}

function InfoRow({ label, value, valueColor = 'text-gray-900' }: { label: string; value: string; valueColor?: string }) {
  return (
