...
test.describe('📦 상품 목록 페이지', () => {
    ...
    // 🔹 기본 로딩 테스트
    test('기본 페이지 로딩 및 상품 목록 표시', async ({ page, productHelpers }) => {
    ...
    test('유형 필터 - BOOK 만 표시', async ({ page, productHelpers }) => {
        ...
        const ebookCard = page.locator(`[data-testid="product-card-${MOCK_PRODUCTS.abcEbook.id}"]`);
    ...
    test('유형 필터 - EBOOK 만 표시', async ({ page, productHelpers }) => {
        ...
        // BOOK 상품 미표시 확인
        await expect(page.getByText(MOCK_PRODUCTS.springIntro.name)).not.toBeVisible();
    });
    ...
    test('상태 필터 - 판매중만 표시', async ({ page, productHelpers }) => {
        ...
        // 판매중 상품 포함 확인
        await expect(page.locator('[data-testid="status-badge-SELLING"]').first()).toBeVisible();

        // 품절 상품은 제외 (품절 상품만 있는 경우)
        const soldOutCard = page.locator(`[data-testid="product-card-${MOCK_PRODUCTS.soldOut.id}"]`);
    ...
    test('상태 필터 - 품절만 표시', async ({ page, productHelpers }) => {
        ...
        // 품절 뱃지 표시 확인
        await expect(page.locator('[data-testid="status-badge-SOLD_OUT"]').first()).toBeVisible();
    });
    ...
    test('복합 필터 - BOOK + 판매중', async ({ page, productHelpers }) => {
        ...
        // ✅ URL 에 두 파라미터 모두 포함
        await expect(page).toHaveURL(/type=BOOK.*status=SELLING|status=SELLING.*type=BOOK/);
    ...
    // 🔹 페이지네이션: 다음 페이지
    test('페이지네이션 - 다음 페이지 이동 시 파라미터 유지', async ({ page, productHelpers }) => {
    ...
    // 🔹 상품 카드 클릭 → 상세 페이지 이동
    test('상품 카드 클릭 시 상세 페이지로 이동', async ({ page, productHelpers }) => {
        await productHelpers.goToProductList();
