...
test.describe("🛍️ 판매자 전용 상품 생성 페이지", () => {
  ...
  test("비로그인으로 접근 시 로그인 페이지로 리다이렉트", async ({
    page,
    productHelpers,
  }) => {
  ...
  test("구매자 (BUYER) 로 접근 시 403 에러", async ({
    page,
    productHelpers,
    loginAs,
  }) => {
  ...
  test("판매자 (SELLER) 로 접근 시 페이지 정상 렌더링", async ({
    page,
    productHelpers,
    loginAsSeller,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "새 상품 등록" }),
    ).toBeVisible();
  ...
  test("상품명 비어있을 때 서버 검증 에러 (1~100자)", async ({
    page,
    productHelpers,
    loginAsSeller,
  }) => {
  ...
  test("가격이 음수일 때 에러 (0 이상의 정수)", async ({
    page,
    productHelpers,
    loginAsSeller,
  }) => {
  ...
  test("유효한 데이터로 상품 생성 성공 (응답 포맷 검증)", async ({
    page,
    productHelpers,
    loginAsSeller,
  }) => {
    ...
    // ✅ 생성된 상품 페이지에서 내용 확인
    await expect(
      page.getByRole("heading", { name: testData.productName }),
    ).toBeVisible();
    await expect(
      page.getByText(`${testData.price.toLocaleString()}원`),
    ).toBeVisible();
  });

  test("선택 필드 (description, imageUrl) 생략 시에도 생성 성공", async ({
    page,
    productHelpers,
    loginAsSeller,
  }) => {
    ...
    await expect(page).toHaveURL(/\/products\/\d+/);
    await expect(
      page.getByRole("heading", { name: "최소 필수 상품" }),
    ).toBeVisible();
  });

  test("description 이 빈 문자열일 때 정상 처리", async ({
    page,
    productHelpers,
    loginAsSeller,
  }) => {
  ...
  test("취소 버튼 클릭 시 이전 페이지로 이동", async ({
    page,
    productHelpers,
    loginAsSeller,
  }) => {
    ...
    await page.evaluate(() =>
      window.history.pushState({}, "", "/seller/products"),
    );
