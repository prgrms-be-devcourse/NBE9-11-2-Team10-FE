...
export const test = base.extend<{
    productHelpers: {
        ...
        assertProductCard: (page: Page, product: typeof MOCK_PRODUCTS.springIntro) => Promise<void>;
        applyFilter: (page: Page, type: 'type' | 'status', value: string) => Promise<void>;
        goToCreateProductPage: () => Promise<void>;
        fillProductForm: (page: Page, data: Partial<MockProductFormData>) => Promise<void>;
        submitProductForm: (page: Page) => Promise<void>;
        assertFormError: (page: Page, fieldName: string, errorMessage: string) => Promise<void>;
    ...
    productHelpers: async ({ page }, use) => {
        const helpers = {
            ...
            goToProductList: async (params?: Record<string, string>) => {
                const url = new URL('/products', process.env.BASE_URL || 'http://localhost:3000');
                if (params) {
            ...
            // 🔹 상품 카드 내용 검증 (목록 페이지용)
            assertProductCard: async (page: Page, product: typeof MOCK_PRODUCTS.springIntro) => {
                const card = page.locator(`[data-testid="product-card-${product.id}"]`);

                await expect(card).toBeVisible();
                await expect(card.getByText(product.name)).toBeVisible();
                await expect(card.getByText(`${product.price.toLocaleString()}원`)).toBeVisible();
            ...
            // 🔹 필터 적용 (클릭 기반)
            applyFilter: async (page: Page, filterType: 'type' | 'status', value: string) => {
                const button = page.locator(`[data-testid="filter-${filterType}-${value}"]`);
                await button.click();
                await page.waitForLoadState('networkidle');
            },
            // ✅ 판매자 상품 생성 페이지 이동
            ...
            // ✅ 상품 폼 채우기
            fillProductForm: async (page: Page, data: Partial<MockProductFormData>) => {
                // ✅ 1. 폼 요소들이 하이드레이션될 때까지 대기 (최대 5 초)
                await page.waitForSelector('#productName', { state: 'visible', timeout: 5000 });
            ...
            submitProductForm: async (page: Page) => {
                const submitButton = page.getByRole('button', { name: /등록하기|처리 중/ });
                await submitButton.click();
                await page.waitForLoadState('networkidle');
            },

            // ✅ 폼 에러 메시지 검증
            assertFormError: async (page: Page, fieldName: string, errorMessage: string) => {
                // ✅ 에러 메시지 텍스트 확인
                await expect(page.getByText(errorMessage)).toBeVisible();

                // ✅ 입력 필드 에러 스타일 확인 (Tailwind 클래스)
                const input = page.locator(`#${fieldName}, [name="${fieldName}"]`);
                await expect(input).toHaveClass(/border-red-500|bg-red-50|focus:ring-red-500/);
            },
            // ✅ Mock 데이터 초기화 (E2E 테스트 격리용)
            resetMockProducts: async () => {
                await page.request.delete(
                    `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/v1/stores/me/products/__reset`
                );
            },
    ...
    loginAs: async ({ page }, use) => {
        const loginAs = async (user: MockUser) => {
            ...
            if (!hasAccessToken) {
                console.warn('⚠️ accessToken 쿠키가 발급되지 않았습니다. 서버 설정을 확인하세요.');
            }
        };
        await use(loginAs);
    },
