...
test.describe('쿠키 포워딩 검증 (Next.js 라우터 통함)', () => {
    ...
    test('디버그 엔드포인트가 정상적으로 응답하는지 먼저 확인', async ({ page }) => {
        // 🔹 1. 상태 코드 먼저 확인
        const response = await page.request.get(`${NEXT_APP}/api/for-debug/forward-headers`);
    ...
    test('브라우저 쿠키가 Next.js 를 통해 백엔드 형식으로 포워딩된다', async ({ page }) => {
    ...
    test('쿠키가 없을 경우 Cookie 헤더가 포함되지 않는다', async ({ page }) => {
      ...
      const response = await newPage.request.get(`${NEXT_APP}/api/for-debug/forward-headers`);
      const body = await response.json();
